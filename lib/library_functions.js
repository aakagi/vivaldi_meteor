...
getStudentTasks = function() {
    return TasksData.find({userId: Meteor.userId()});
}
            ...
            for (j in tasks) {
                if (taskIds.indexOf(tasks[j]._id) < 0) {
                    ...
                    for (k in tasks[j].sections) {
                        var section = Sections.findOne({_id: tasks[j].sections[k]});
...
getFullStudentTasks = function(incomplete, complete, typeBoolean, sectionData) {
    ...
    if (incomplete && complete) {
        var selector = {userId: Meteor.userId()};
    } else if (incomplete) {
    ...
    for (i in td) {
        ...
        var ti = Tasks.findOne({_id: td[i].taskId});
        ...
        if (sectionData) {
            ...
            for (k in task.sections) {
                sections.push(Sections.findOne({_id: task.sections[k]}));
            }
